...
export class CreateDummyData1759636753110 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
           ...
           ('FoodCorp Industries', (SELECT user_id FROM public.users WHERE email = 'lisa.m@foodcorp.com')),
           ('Healthy Foods Co', (SELECT user_id FROM public.users WHERE email = 'david.a@healthyfoods.com')),
           ('Organic Suppliers LLC', (SELECT user_id FROM public.users WHERE email = 'jennifer.t@organic.com'))
       `);
    ...
    await queryRunner.query(`
           ...
           (
               (SELECT food_manufacturer_id FROM public.food_manufacturers WHERE food_manufacturer_name = 'FoodCorp Industries'),
               '2024-01-15 10:30:00',
               'available',
               150,
               2400.50,
           ...
           (
               (SELECT food_manufacturer_id FROM public.food_manufacturers WHERE food_manufacturer_name = 'Healthy Foods Co'),
               '2024-01-20 14:00:00',
               'partially_allocated',
               200,
               3200.00,
           ...
           (
               (SELECT food_manufacturer_id FROM public.food_manufacturers WHERE food_manufacturer_name = 'Organic Suppliers LLC'),
               '2024-01-25 09:15:00',
               'available',
               100,
               1600.75,
    ...
    await queryRunner.query(`
           ...
           (
               (SELECT pantry_id FROM public.pantries WHERE pantry_name = 'Community Food Pantry Downtown'),
               'large',
           ...
           (
               (SELECT pantry_id FROM public.pantries WHERE pantry_name = 'Westside Community Kitchen'),
               'medium',
           ...
           (
               (SELECT pantry_id FROM public.pantries WHERE pantry_name = 'North End Food Bank'),
               'small',
    ...
    await queryRunner.query(`
           ...
           (
               (SELECT order_id FROM public.orders WHERE status = 'delivered' AND shipped_at = '2024-01-17 08:00:00'),
               (SELECT item_id FROM public.donation_items WHERE item_name = 'Peanut Butter (16oz)'),
               10,
               '2024-01-16 09:00:00',
               '2024-01-18 14:30:00',
               'fulfilled'
           ...
           (
               (SELECT order_id FROM public.orders WHERE status = 'delivered' AND shipped_at = '2024-01-17 08:00:00'),
               (SELECT item_id FROM public.donation_items WHERE item_name = 'Canned Green Beans'),
               5,
               '2024-01-16 09:00:00',
               '2024-01-18 14:30:00',
               'fulfilled'
           ...
           (
               (SELECT order_id FROM public.orders WHERE status = 'delivered' AND shipped_at = '2024-01-22 09:00:00'),
               (SELECT item_id FROM public.donation_items WHERE item_name = 'Gluten-Free Pasta'),
               30,
               '2024-01-21 10:00:00',
               '2024-01-23 10:00:00',
               'fulfilled'
           ...
           (
               (SELECT order_id FROM public.orders WHERE status = 'delivered' AND shipped_at = '2024-01-22 09:00:00'),
               (SELECT item_id FROM public.donation_items WHERE item_name = 'Almond Milk'),
               20,
               '2024-01-21 10:00:00',
               '2024-01-23 10:00:00',
               'fulfilled'
           ...
           (
               (SELECT order_id FROM public.orders WHERE status = 'pending'),
               (SELECT item_id FROM public.donation_items WHERE item_name = 'Cereal Boxes'),
               75,
               '2024-02-03 12:00:00',
               NULL,
               'pending'
    ...
    await queryRunner.query(`
           ...
           (
               (SELECT pantry_id FROM public.pantries WHERE pantry_name = 'Community Food Pantry Downtown'),
               (SELECT user_id FROM public.users WHERE email = 'james.t@volunteer.org')
           ),
           (
               (SELECT pantry_id FROM public.pantries WHERE pantry_name = 'Community Food Pantry Downtown'),
               (SELECT user_id FROM public.users WHERE email = 'patricia.j@volunteer.org')
           ),
           (
               (SELECT pantry_id FROM public.pantries WHERE pantry_name = 'Westside Community Kitchen'),
               (SELECT user_id FROM public.users WHERE email = 'maria.g@volunteer.org')
           ),
           (
               (SELECT pantry_id FROM public.pantries WHERE pantry_name = 'North End Food Bank'),
               (SELECT user_id FROM public.users WHERE email = 'william.m@volunteer.org')
           ),
           (
               (SELECT pantry_id FROM public.pantries WHERE pantry_name = 'North End Food Bank'),
               (SELECT user_id FROM public.users WHERE email = 'maria.g@volunteer.org')
