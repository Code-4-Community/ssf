...
// Recurrence settings handed up on submit; null when the donation is not recurring
...
interface EditableDonationItemsTableProps {
...
const EditableDonationItemsTable: React.FC<EditableDonationItemsTableProps> = ({
  ...
  const handleSubmit = () => {
    ...
    const recurrence: RecurrenceData | null =
      showRecurrence && isRecurring
        ? {
            recurrenceFreq: parseInt(repeatEvery),
            recurrence: repeatInterval,
            repeatOnDays:
              repeatInterval === RecurrenceEnum.WEEKLY ? repeatOn : undefined,
            occurrencesRemaining: parseInt(endsAfter),
          }
        : null;

    onSubmit(rows, recurrence);
  };
  ...
  return (
    <>
              ...
              {showRecurrence && (
                <Checkbox.Root
                  ...
                  onCheckedChange={(e: { checked: boolean }) => {
                    ...
                    setRepeatInterval(
                      e.checked ? RecurrenceEnum.WEEKLY : RecurrenceEnum.NONE,
                    );
                  }}
                >
          ...
          >
                  ...
                  <Portal>
                    <Menu.Positioner>
                      <Menu.Content
                        ...
                        zIndex={9999}
                      >
                        {(Object.keys(repeatOn) as DayOfWeek[]).map((day) => (
                          <Menu.Item
                            key={day}
                            value={day}
            ...
            <Box flex="1" minW="200px">
              ...
              <NumberInput.Root
                width="100%"
                value={endsAfter}
                onValueChange={(e: { value: string }) => setEndsAfter(e.value)}
                min={1}
                step={1}
              >
      ...
      <Flex justifyContent="flex-end" gap={3} mt={6} pt={4} align="center">
        <Button
          variant="outline"
          color="gray.700"
          fontWeight={600}
        ...
        <Tooltip.Root disabled={firstValidationError === null} openDelay={0}>
          ...
          <Portal>
            <Tooltip.Positioner>
              <Tooltip.Content>{firstValidationError ?? ''}</Tooltip.Content>
            </Tooltip.Positioner>
          </Portal>
        </Tooltip.Root>
      </Flex>
...
export default EditableDonationItemsTable;
