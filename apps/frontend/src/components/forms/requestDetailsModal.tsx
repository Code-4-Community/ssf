...
import { FoodRequest, FoodTypes, OrderDetails, OrderItemDetails } from "types/types"
...
const RequestDetailsModal: React.FC<RequestDetailsModalProps> = ({
    ...
    useEffect(() => {
        const fetchRequestOrderDetails = async () => {
            try {
                const orderDetailsList = await apiClient.getOrderDetailsListFromRequest(request.requestId);
                const sortedData = orderDetailsList.slice().sort((a, b) => b.orderId - a.orderId);
    ...
    const groupedOrderItemsByType = useMemo(() => {
        ...
        return currentOrder.items.reduce((acc: Record<(typeof FoodTypes)[number], OrderItemDetails[]>, item) => {
                ...
                <Dialog.Content maxW={650}>
                  ...
                  <Dialog.Body>
                    ...
                    <Tabs.Root mt={5} defaultValue="request details">
                        <Tabs.List>
                            <Tabs.Trigger textStyle="p2" color="neutral.800" value="request details">
                            Request Details
                            </Tabs.Trigger>
                            <Tabs.Trigger textStyle="p2" color="neutral.800" value="associated orders">
                            Associated Orders
                            </Tabs.Trigger>
                        </Tabs.List>
                        <Tabs.Content value="request details">
                            ...
                            <Field.Root mb={4} mt={3}>
                                <Field.Label>
                                <Text textStyle="p2" fontWeight={600} color="neutral.800" mt={3}>
                                    Food Type(s)
                                </Text>
                                </Field.Label>
                            ...
                            >
                                <Flex justify="space-between" align="center" mb={3}>
                                    ...
                                    {currentOrder.status === OrderStatus.DELIVERED ? (
                                        <Badge py={1} px={2} bgColor="#EAEDEF" color="#2B4E60" textStyle="p2" fontWeight={500}>Received</Badge>
                                    ) : (
                                        <Badge py={1} px={2} bgColor="#FEECD1" color="#9C5D00" textStyle="p2" fontWeight={500}>In Progress</Badge>
                                    )}
                                </Flex>
                                {Object.entries(groupedOrderItemsByType as Record<string, OrderItemDetails[]>).map(([foodType, items]) => (
                                    <Box key={foodType} mb={4}>
                                        ...
                                        <Flex
                                            ...
                                            mt="2"
                                            >
                                            <Text py={2} textStyle="p2" color="neutral.800" flex={1}>{item.name}</Text>
                                            ...
                                            <Text py={2} textStyle="p2" color="neutral.800">{item.quantity}</Text>
                                ...
                                <Text color="neutral.800" textStyle="p2" fontWeight={600} mt="3">Tracking</Text>
                                <Text color="neutral.700" textStyle="p2" mt="3" mb="3">No tracking link available at this time</Text>
                            </Box>
                            )}

                            <Flex justify="center" mt={12}>
                                <Pagination.Root count={orderDetailsList.length} pageSize={1} page={currentPage} onChange={(page) => setCurrentPage(page)}>
                                    ...
                                    <Pagination.NextTrigger asChild>
                                        <IconButton variant="ghost" onClick={() => setCurrentPage((prev) => Math.min(prev + 1, Math.ceil(orderDetailsList.length)))}>
                                        <ChevronRight />
                                        </IconButton>
